...
export const select =
    ...
    (req: Request, res: Response, next: NextFunction, ...rest: unknown[]) => {
        ...
        if (typeof selector === "number") {
            ...
            if (index < 0 || index > middlewares.length - 1)
                throw new Error(
                    "The selector function returns a number that is out of bounds"
                );
            middleware = middlewares[index];
        } else {
            throw new Error(
                "The selector shoul be either a number or a function that return a number"
            );
        }
        if (middleware) return middleware(req, res, next, ...rest);
        else return next();
    };
